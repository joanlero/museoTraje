...
class WebMainUpdateTecnica extends StatefulWidget {
  static const String idRoute = "updatetecnica";

  WebMainUpdateTecnica({Key? key, this.id, this.tecnica}) : super(key: key);

  final String? id;
  final ModelTecnicasWeb? tecnica;

  @override
  State<WebMainUpdateTecnica> createState() => _WebMainUpdateTecnicaState();
}

class _WebMainUpdateTecnicaState extends State<WebMainUpdateTecnica> {
  ...
  void initState() {
    super.initState();
    if (widget.tecnica != null) {
      titleC.text = widget.tecnica!.nombreTecnica!;
      descriptionShortC.text = widget.tecnica!.descriptionShortTecnica!;
      descriptionLargeC.text = widget.tecnica!.descriptionLargeTecnica!;
    }
  }
  ...
  Widget build(BuildContext context) {
    return AdminScaffold(
      ...
      sideBar: SideBar(
        ...
        selectedRoute: WebMainUpdateTecnica.idRoute,
        onSelected: (item) {
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              maxLength: 2000,
                              decoration: InputDecoration(
                                hintText: 'Descripción Larga de la carrera técnica',
                                border: OutlineInputBorder(
                                  borderSide: BorderSide(color: Colors.black),
                                ),
                                enabledBorder: OutlineInputBorder(
  ...
  save() async {
    if (_formKey.currentState!.validate()) {
      ...
      await uploadImages();
      await TecnicasControllerWeb.updateTecnica(
        widget.id!,
        ModelTecnicasWeb(
          idTecnica: widget.id,
          nombreTecnica: titleC.text,
          descriptionShortTecnica: descriptionShortC.text,
          descriptionLargeTecnica: descriptionLargeC.text,
          imageUrlsTecnica: imageUrlsTecnica,
  ...
  Future<void> uploadImages() async {
    for (var image in images) {
      await postImages(image).then((downLoadUrl) {
        if (imageUrlsTecnica.isNotEmpty) {
          imageUrlsTecnica += ';' + downLoadUrl!;
        } else {
          imageUrlsTecnica = downLoadUrl!;
        }
      });
    }
    Navigator.of(context).pushNamed(ListarTecnicaScreen.id);
  }
}
