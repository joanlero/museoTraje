...
class WebMainUpdatePosgrado extends StatefulWidget {
  static const String idRoute = "updatePosgrado";

  WebMainUpdatePosgrado({Key? key, this.id, this.posgrado}) : super(key: key);

  final String? id;
  final ModelPosgradoWeb? posgrado;

  @override
  State<WebMainUpdatePosgrado> createState() => _WebMainUpdatePosgradoState();
}

class _WebMainUpdatePosgradoState extends State<WebMainUpdatePosgrado> {
  ...
  void initState() {
    super.initState();
    if (widget.posgrado != null) {
      titleC.text = widget.posgrado!.nombrePosgrado!;
      descriptionShortC.text = widget.posgrado!.descriptionShortPosgrado!;
      descriptionLargeC.text = widget.posgrado!.descriptionLargePosgrado!;
    }
  }
  ...
  Widget build(BuildContext context) {
    return AdminScaffold(
      ...
      sideBar: SideBar(
        ...
        selectedRoute: WebMainUpdatePosgrado.idRoute,
        onSelected: (item) {
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              maxLength: 2000,
                              decoration: InputDecoration(
                                hintText: 'Descripción Larga de la posgrado',
                                border: OutlineInputBorder(
                                  borderSide: BorderSide(color: Colors.black),
                                ),
                                enabledBorder: OutlineInputBorder(
  ...
  save() async {
    if (_formKey.currentState!.validate()) {
      ...
      await uploadImages();
      await PosgradoControllerWeb.updatePosgrado(
        widget.id!,
        ModelPosgradoWeb(
          idPosgrado: widget.id,
          nombrePosgrado: titleC.text,
          descriptionShortPosgrado: descriptionShortC.text,
          descriptionLargePosgrado: descriptionLargeC.text,
          imageUrls: imageUrls,
  ...
  Future<void> uploadImages() async {
    ...
    Navigator.of(context).pushNamed(ListarPosgradoScreen.id);
  }
}
