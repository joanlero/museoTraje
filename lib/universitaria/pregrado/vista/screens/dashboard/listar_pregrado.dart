...
import 'package:adminmuseo/universitaria/pregrado/controller/pregradoControllerWeb.dart';
import 'package:adminmuseo/universitaria/pregrado/model/modelPregrado.dart';
import 'package:adminmuseo/universitaria/pregrado/vista/screens/dashboard/register_pregrado.dart';
import 'package:adminmuseo/universitaria/pregrado/vista/screens/dashboard/update_pregrado.dart';
...
class ListarPregradoScreen extends StatefulWidget {
  static const String id = "listarpregrado";

  @override
  State<ListarPregradoScreen> createState() => _ListarPregradoScreenState();
}

class _ListarPregradoScreenState extends State<ListarPregradoScreen> {
  String searchQuery = "";
  ...
  void filterDocuments(String query) {
    setState(() {
      ...
      filteredDocuments = allDocuments.where((doc) {
        final nombrePregrado = doc['nombrePregrado'].toString().toLowerCase();
        return nombrePregrado.contains(searchQuery);
      }).toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    return AdminScaffold(
      ...
      sideBar: SideBar(
        ...
        selectedRoute: ListarPregradoScreen.id,
        onSelected: (item) {
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ElevatedButton(
                                onPressed: () => registerPregrado(context),
                                ...
                                child: Container(
                                  ...
                                  child: TextField(
                                    decoration: InputDecoration(
                                      labelText: 'Buscar por nombre de pregrados',
                          ...
                          Expanded(
                            child: filteredDocuments.isNotEmpty
                              ...
                              source: PregradoDataTableSource(data: filteredDocuments, context: context),
                            )
                                : Center(
  ...
  void registerPregrado(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => WebMainRegisterPregrado()),
    );
  }
}

class   PregradoDataTableSource extends DataTableSource {
  final List<QueryDocumentSnapshot> data;
  final BuildContext context;

  PregradoDataTableSource({required this.data, required this.context});

  @override
  DataRow getRow(int index) {
    final pregradoIndex = data[index];
    return DataRow2(
      cells: [
        DataCell(
          Text(
            pregradoIndex['nombrePregrado'] ?? '',
            overflow: TextOverflow.ellipsis,
          ),
        ),
        DataCell(
          Text(
            pregradoIndex['descriptionShortPregrado'] ?? '',
            overflow: TextOverflow.ellipsis,
          ),
        ),
        DataCell(
          Text(
            pregradoIndex['descriptionLargePregrado'] ?? '',
            overflow: TextOverflow.ellipsis,
          ),
        ),
        DataCell(
              ...
              IconButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => WebMainUpdatePregrado(
                        id: pregradoIndex.id,
                        pregrado: ModelPregradoWeb(
                          idPregrado: pregradoIndex['idPregrado'],
                          nombrePregrado: pregradoIndex['nombrePregrado'],
                          descriptionShortPregrado: pregradoIndex['descriptionShortPregrado'],
                          descriptionLargePregrado: pregradoIndex['descriptionLargePregrado'],
                          imageUrlsPregrado: pregradoIndex['imageUrlsPregrado'],
                      ...
                      return AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          TextButton(
                            child: Text('Eliminar'),
                            onPressed: () async {
                              await PregradoControllerWeb.deletePregrado(pregradoIndex.id);
                              Navigator.of(context).pop();
                              // Redirigir a ListarMuseoScreen después de eliminar
                              Navigator.of(context).pushReplacementNamed(ListarPregradoScreen.id);
                            },
                          ),
                        ],
                      );
