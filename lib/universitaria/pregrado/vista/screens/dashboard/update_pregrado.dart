...
class WebMainUpdatePregrado extends StatefulWidget {
  static const String idRoute = "updatepregrado";

  WebMainUpdatePregrado({Key? key, this.id, this.pregrado}) : super(key: key);

  final String? id;
  final ModelPregradoWeb? pregrado;

  @override
  State<WebMainUpdatePregrado> createState() => _WebMainUpdatePregradoState();
}

class _WebMainUpdatePregradoState extends State<WebMainUpdatePregrado> {
  ...
  void initState() {
    super.initState();
    if (widget.pregrado != null) {
      titleC.text = widget.pregrado!.nombrePregrado!;
      descriptionShortC.text = widget.pregrado!.descriptionShortPregrado!;
      descriptionLargeC.text = widget.pregrado!.descriptionLargePregrado!;
    }
  }
  ...
  Widget build(BuildContext context) {
    return AdminScaffold(
      ...
      sideBar: SideBar(
        ...
        selectedRoute: WebMainUpdatePregrado.idRoute,
        onSelected: (item) {
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              maxLength: 2000,
                              decoration: InputDecoration(
                                hintText: 'Descripción Larga del pregrado',
                                border: OutlineInputBorder(
                                  borderSide: BorderSide(color: Colors.black),
                                ),
                                enabledBorder: OutlineInputBorder(
  ...
  save() async {
    if (_formKey.currentState!.validate()) {
      ...
      await uploadImages();
      await PregradoControllerWeb.updatePregrado(
        widget.id!,
        ModelPregradoWeb(
          idPregrado: widget.id,
          nombrePregrado: titleC.text,
          descriptionShortPregrado: descriptionShortC.text,
          descriptionLargePregrado: descriptionLargeC.text,
          imageUrlsPregrado: imageUrlsPregrado,
  ...
  Future<void> uploadImages() async {
    for (var image in images) {
      await postImages(image).then((downLoadUrl) {
        if (imageUrlsPregrado.isNotEmpty) {
          imageUrlsPregrado += ';' + downLoadUrl!;
        } else {
          imageUrlsPregrado = downLoadUrl!;
        }
      });
    }
    Navigator.of(context).pushNamed(ListarPregradoScreen.id);
  }
}
