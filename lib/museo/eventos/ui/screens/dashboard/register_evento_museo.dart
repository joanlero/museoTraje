...
class _WebMainRegisterEventoMuseoState extends State<WebMainRegisterEventoMuseo> {
  Widget selectedScreen = WebMainRegisterEventoMuseo();
  ...
  String imageUrlsEvento =
      ''; // Cambio aquí: inicializo como cadena vacía en lugar de null
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            validator: (v) {
                              if (v!.isEmpty) {
                                return "Titulo del Evento no puede estar Vacio";
                              }
                              return null;
                            },
                            controller: titleEC,
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              validator: (v) {
                                if (v!.isEmpty) {
                                  return "Descripción corta del Evento no puede estar Vacio";
                                }
                                return null;
                              },
                        ...
                        Expanded(
                          child: SizedBox(
                            ...
                            child: ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                backgroundColor:
                                Color(0xFF003F5D), // Color del botón
                              ),
                              child: isLoading
  ...
  save() async {
    if (_formKey.currentState!.validate()) {
      ...
      await uploadImages();
      await ModelEventoMuseoTraje.addEventoMuseo (ModelEventoMuseoTraje(
          idEvento: uuid.v4(),
  ...
  uploadImages() async {
    for (var image in images) {
      await postImages(image).then((downLoadUrl) {
        if (imageUrlsEvento.isNotEmpty) {
          imageUrlsEvento +=
              ';' + downLoadUrl; // Agrega un punto y coma como separador
        } else {
    ...
    // Luego de cargar todas las imágenes, navega a la siguiente pantalla
